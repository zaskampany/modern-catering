...
// ============ MAP ============
// NEXT_PUBLIC_MAP_EMBED_URL  Google Maps → Share → "Embed a map" → copy the
//                            src="..." out of the iframe it gives you. A plain
//                            /maps/place/ or goo.gl link will NOT render in an
//                            iframe, so it falls back to the address query
//                            below, which always works and needs no API key.
// NEXT_PUBLIC_MAP_LINK       Where the "Get directions" button points.
const mapQuery = encodeURIComponent(
  process.env.NEXT_PUBLIC_MAP_QUERY ||
    `${site.addressLine1}, ${site.addressLine2}`
);
const embedFromEnv = (process.env.NEXT_PUBLIC_MAP_EMBED_URL || "").trim();
// "lat,lng" — pins the marker exactly, which an address search only approximates.
const mapCoords = (process.env.NEXT_PUBLIC_MAP_COORDS || "").trim();

function resolveEmbed() {
  // Only a true /maps/embed URL can be an iframe src. A /maps/place/ or
  // goo.gl link renders as "refused to connect", so anything else falls
  // through to the query form, which always works and needs no API key.
  if (/^https:\/\/(www\.)?google\.[^/]+\/maps\/embed/.test(embedFromEnv)) return embedFromEnv;
  if (/^-?\d+(\.\d+)?\s*,\s*-?\d+(\.\d+)?$/.test(mapCoords)) {
    return `https://maps.google.com/maps?q=${encodeURIComponent(mapCoords)}&z=17&output=embed`;
  }
...
export const map = {
  ...
  link:
    ...
    `https://www.google.com/maps/search/?api=1&query=${mapCoords || mapQuery}`,
};
...
// Payasam is ordered on its own, not as an add-on, so it needs a second Google
// Form — the sadhya one makes the package count a required question.
const onamPayasamFormUrl = process.env.NEXT_PUBLIC_ONAM_PAYASAM_FORM_URL || "";
const onamBookingPhone = process.env.NEXT_PUBLIC_ONAM_BOOKING_PHONE || "+919497843441";
// Renamed from _DELIVERY_DATE when the offer became collection-only; the old
// name is still honoured so an un-updated .env keeps working.
const onamCollectionDate =
  process.env.NEXT_PUBLIC_ONAM_COLLECTION_DATE ||
  process.env.NEXT_PUBLIC_ONAM_DELIVERY_DATE ||
  "2026-08-26";
...
export const onam = {
  ...
  payasamFormUrl: onamPayasamFormUrl,
  payasamFormPostUrl: toPostUrl(onamPayasamFormUrl),
  ...
  poster: "/images/onam-poster-2026.webp",
  ...
  // Collection happens on Thiruvonam day only. There is no delivery — every
  // order is picked up from the address below.
  collectionDate: onamCollectionDate,
  collectionDateLabel: formatDate(onamCollectionDate),

  // Collection point and timings — this is a pickup offer, not on-site
  // catering, so these matter more than they would for an event booking.
  pickup: process.env.NEXT_PUBLIC_ONAM_PICKUP || "Modern Catering, Kolazhy, Doctor Padi",
